...
@Directive({
  ...
  selector: 'input[smasTrim], textarea[smasTrim]',
  providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: SmasTrimDirective, multi: true }]
})
export class SmasTrimDirective implements ControlValueAccessor {
  private get _type(): string {
    return this._sourceElementRef.nativeElement.type || 'text';
  }

  // Get a value of the trim attribute if it was set.
  @Input() trim: string = 'blur';
  ...
  onBlur(event: string, value: string): void {
    this.updateValue(event, value.replace(/(\s\s+| )/g, ' ').trim());
    this.onTouched();
  }
  ...
  private updateValue(event: string, value: string): void {
    ...
    value = this.trim !== '' && event !== this.trim ? value : value.replace(/(\s\s+| )/g, ' ').trim();
    ...
    //
    if ((this._value || previous) && this._value.replace(/(\s\s+| )/g, ' ').trim() !== previous) {
