...
import { selectGrantedPolicies } from '../../../core/auth/_selectors/config.selectors';
...
export class BaseComponent implements OnInit, OnDestroy {
  ...
  loadRolesWithPermissions() {
    ...
    this.currentUserPermissions$ = this.store.pipe(select(selectGrantedPolicies));
    const subscription = this.currentUserPermissions$.subscribe(res => {
      ...
      for(const key in res){
        if(res.hasOwnProperty(key)){
          this.permissionsService.addPermission(key.toString());
        }
      }
    });
    this.unsubscribe.push(subscription);
